...
#include <queue>
#include <iomanip> // ✨ 추가된 부분: 16진수 출력을 위해 필요
#include <sstream> // ✨ 추가된 부분: 16진수 출력을 위해 필요
...
void can_to_doip_forwarder(int can_sock, int client_sock, std::atomic<bool>& session_active) {
    ...
    while (session_active) {
        ...
        // ✨ 추가된 부분: 수신된 UDS 응답 데이터 출력
        print_hex("    ECU -> RPi (UDS Response)", uds_response);
...
void handle_doip_session(int client_sock) {
    ...
    while (session_active) {
        ...
        if (payload_type == 0x0005) {
            ...
            resp_header->payload_type = htons(0x0006);
            resp_header->payload_length = htonl(9);
            ...
            *client_addr_ptr = htons(CLIENT_LOGICAL_ADDRESS);
            *server_addr_ptr = htons(SERVER_LOGICAL_ADDRESS);
            *response_code_ptr = 0x10;
        ...
        *ack_sa = htons(SERVER_LOGICAL_ADDRESS);
        *ack_ta = htons(CLIENT_LOGICAL_ADDRESS);
        *ack_code = 0x00;
        ...
        std::cout << "\nDoIP: Positive ACK (0x8002) 전송 완료" << std::endl;

        std::vector<uint8_t> uds_request(payload_buffer.begin() + 4, payload_buffer.end());
        std::cout << "[DoIP -> CAN] DoIP로부터 UDS 요청 수신 (" << uds_request.size() << " 바이트)" << std::endl;

        // ✨ 추가된 부분: 전송할 UDS 요청 데이터 출력
        print_hex("    PC -> RPi (UDS Request)", uds_request);

        isotp_send(can_sock, UDS_REQUEST_CAN_ID, uds_request);
    }

    session_active = false;
    fc_queue_cv.notify_all();
...
// --- ISO-TP 수신 메인 함수 (스레드에서 직접 사용하므로 이 함수는 이제 호출되지 않음) ---
std::vector<uint8_t> isotp_receive(int sock) {
    can_frame rx_frame;
    ...
    uint8_t pci_type = (rx_frame.data[0] & 0xF0) >> 4;
    switch (pci_type) {
    case 0: return isotp_handle_single_frame(rx_frame);
    case 1: return isotp_handle_multi_frame(sock, rx_frame);
    default:
        std::cerr << "잘못된 시작 프레임 수신 (Type: " << (int)pci_type << ")" << std::endl;
        return {};
    }
...
void isotp_send_flow_control(int sock) {
    can_frame fc_frame;
    fc_frame.can_id = UDS_REQUEST_CAN_ID;
    fc_frame.can_dlc = 8;
